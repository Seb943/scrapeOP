...
DRIVER_LOCATION = "C:\\Users\\Utilisateur\\Desktop\\chromedriver.exe" # for mac/unix use "chromedriver" and copy chromediver file to usr/local/bin
...
def get_data_typeA(i, link):
    ...
    if a == True:
        ...
        for j in range(1, 30):  # only first 10 bookmakers displayed
            ...
            Book = ffi(
                '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[1]/div/a[2]'.format(j))
            # first home odd
            Odd_1 = fffi(
                '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[2]/div'.format(j))
            # first away odd
            Odd_2 = fffi(
                '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[3]/div'.format(j))
...
def scrape_page_next_games_typeA(country, sport, tournament, nmax=20):
    link = 'https://www.oddsportal.com/{}/{}/{}/'.format(
...
def scrape_current_tournament_typeA(sport, tournament, country, SEASON, max_page=25):
    ...
    try:
        data_df.columns = ['TeamsRaw', 'Bookmaker',
                           'OddHome', 'OddAway', 'DateRaw', 'ScoreRaw']
    except:
    ...
    # (c) Split score
    data_df["Score_home"] = [re.split(':', y)[0][-3:]
                             for y in data_df["ScoreRaw"]]
    data_df["Score_away"] = [re.split(':', y)[1][:3]
                             for y in data_df["ScoreRaw"]]
    ...
    data_df.to_csv('./{}_FULL/{}_{}_FULL.csv'.format(tournament,
                   tournament, SEASON), sep=';', encoding='utf-8', index=False)
...
def scrape_current_season_typeA(tournament, sport, country, SEASON, max_page=25):
    ...
    for page in range(1, max_page):
        ...
        try:
            ...
            pass
        driver = webdriver.Chrome(executable_path=DRIVER_LOCATION)
        data = scrape_page_current_season_typeA(
            page, sport, country, tournament)
        DATA_ALL = DATA_ALL + [y for y in data if y != None]
        driver.close()
    data_df = pd.DataFrame(DATA_ALL)
    try:
    ...
    data_df.to_csv('./{}_FULL/{}_CurrentSeason_FULL.csv'.format(tournament,
                   tournament), sep=';', encoding='utf-8', index=False)
    data_df[['Home_id', 'Away_id', 'Bookmaker', 'OddHome', 'OddAway', 'Date', 'Score_home', 'Score_away', 'Season']].\
        to_csv('./{}/{}_CurrentSeason.csv'.format(tournament,
               tournament), sep=';', encoding='utf-8', index=False)
    return (data_df)
...
def scrape_league_typeA(Season, sport, country1, tournament1, nseason, current_season='yes', max_page=25):
    ...
    for i in range(nseason):
        ...
        scrape_current_tournament_typeA(
            sport=sport, tournament=tournament1, country=country1, SEASON=SEASON1, max_page=max_page)
    ...
    if current_season == 'yes':
        ...
        scrape_current_season_typeA(tournament=tournament1, sport=sport,
                                    country=country1, SEASON='CurrentSeason', max_page=max_page)
        print('We finished to collect current season !')

    # Finally we merge all files
    file1 = pd.read_csv('./{}/'.format(tournament1) +
                        os.listdir('./{}/'.format(tournament1))[0], sep=';')
...
def scrape_next_games_typeA(tournament, sport, country, SEASON, nmax=30):
    ...
    if not os.path.exists('./{}'.format(tournament)):
        os.makedirs('./{}'.format(tournament))
    data_df[['Home_id', 'Away_id', 'Bookmaker', 'OddHome', 'OddAway', 'Date', 'Score_home', 'Score_away', 'Season']].to_csv(
        './{}/NextGames_{}_{}_08042020.csv'.format(tournament, tournament, SEASON), sep=';', encoding='utf-8', index=False)
...
def scrape_page_next_games_typeB(country, tournament, nmax=20):
    link = 'https://www.oddsportal.com/tennis/{}/{}/'.format(
        country, tournament)
    DATA = []
    for i in range(1, nmax):
        print(i)
...
def scrape_current_tournament_typeB(Surface, bestof=3, tournament='wta-lyon', country='france', name_to_write='WTA Lyon', SEASON='2020', max_page=20):
    ...
    if not os.path.exists('./{}/{}_{}.csv'.format(tournament, tournament, SEASON)):
        alpha_game = ['We_scrape_full_season',
                      'We_scrape_full_season', 'We_scrape_full_season']
    elif os.path.exists('./{}/{}_{}.csv'.format(tournament, tournament, SEASON)):
        file = pd.read_csv('./{}/{}_{}.csv'.format(tournament,
                           tournament, SEASON), sep=';', encoding='utf-8')
        alpha_game = [file["P1"].iloc[0],
                      file["P2"].iloc[0], file["Date"].iloc[0]]
    ...
    set_scores = [re.split(':', y) for y in [y[5][13:16]
                                             for y in DATA_ALL_CLEANED]]
    DATE = [re.split(', ', y) for y in [y[4] for y in DATA_ALL_CLEANED]]
    games_score = [re.split(',', y) for y in [y[5][18:]
                                              for y in DATA_ALL_CLEANED]]
    ...
    # (0) Filter out None rows
    FINAL_DATASET = FINAL_DATASET[~FINAL_DATASET['OddHome'].isnull(
    )].dropna().reset_index()
    FINAL_DATASET = FINAL_DATASET[~FINAL_DATASET['OddAway'].isnull(
    )].dropna().reset_index()

    if not os.path.exists('./{}'.format(tournament)):
        os.makedirs('./{}'.format(tournament))
    FINAL_DATASET.to_csv('./{}/{}_{}.csv'.format(tournament,
                         tournament, SEASON), sep=';', encoding='utf-8', index=False)
    # print(os.listdir())
    # print(os.getcwd())
...
def scrape_next_games_typeB(Surface, bestof, tournament, country, name_to_write, SEASON='2020'):
    ...
    if not os.path.exists('./{}'.format(tournament)):
        os.makedirs('./{}'.format(tournament))
    FINAL_DATASET.to_csv('./{}/NextGames_{}_{}.csv'.format(tournament,
                         tournament, SEASON), sep=';', encoding='utf-8', index=False)
    # print(os.listdir())
    # print(os.getcwd())
    return (FINAL_DATASET)
...
def get_data_typeC(i, link):
    driver.get(link)
    driver.add_cookie({"name": "oddsportalcom_session", "value": ""})
    time.sleep(2)
    reject_ads()
    target = '//div[contains(@class, \'eventRow\')][{}]/div[contains(@class, \'flex-col\') and contains(@class, \'border-b\')]'.format(i) 
    ...
    if a == True:
        ...
        for j in range(1, 30):  # only first 10 bookmakers displayed
            # first bookmaker name
            Book = ffi(
                '/html/body/div[1]/div/div[1]/div/main/div[2]/div[6]/div[1]/div/div[contains(@class, \'text\')][{}]/div[1]/a[2]/p'.format(j))
            # first home odd
            Odd_1 = fffi(
                '/html/body/div[1]/div/div[1]/div/main/div[2]/div[6]/div[1]/div/div[contains(@class, \'text\')][{}]/div[2]/div/div/p'.format(j))
            Odd_X = fffi(
                '/html/body/div[1]/div/div[1]/div/main/div[2]/div[6]/div[1]/div/div[contains(@class, \'text\')][{}]/div[3]/div/div/p'.format(j))  # draw odd
            # first away odd
            Odd_2 = fffi(
                '/html/body/div[1]/div/div[1]/div/main/div[2]/div[6]/div[1]/div/div[contains(@class, \'text\')][{}]/div[3]/div/div/p'.format(j))
            match = ffi('//*[@id="app"]/div/div[1]/div/main/div[2]/div[4]/ul/li[11]/p')  # match teams
            final_score = ffi('//*[@id="app"]/div/div[1]/div/main/div[2]/div[5]/div[2]/div[3]/div[2]/strong')
            date = ffi('//*[@id="app"]/div/div[1]/div/main/div[2]/div[5]/div[2]/div[1]/div[2]')  # Date and time
            print(match, Book, Odd_1, Odd_X, Odd_2,
                  date, final_score, i, '/ 500 ')
    ...
    if a == True:
        ...
        for j in range(1, 30):  # only first 10 bookmakers displayed
            ...
            Odd_1 = fffi(
                '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[2]/a'.format(j))
            Odd_X = fffi(
                '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[3]/a'.format(j))  # draw odd
            # first away odd
            Odd_2 = fffi(
            ...
            Odd_1 = fffi(
                '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[2]/div'.format(j))
            Odd_X = fffi(
                '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[3]/div'.format(j))  # draw odd
            # first away odd
            Odd_2 = fffi(
                '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[4]/div'.format(j))
            match = ffi('//*[@id="col-content"]/h1')  # match teams
            final_score = ffi('//*[@id="event-status"]')
            date = ffi('//*[@id="col-content"]/p[1]')  # Date and time
            print(match, Book, Odd_1, Odd_X, Odd_2,
...
def scrape_page_typeC(page, sport, country, tournament, SEASON):
    link = 'https://www.oddsportal.com/{}/{}/{}-{}/results/#/page/{}'.format(
        sport, country, tournament, SEASON, page)
    DATA = []
    for i in range(1, 50):
...
def scrape_current_tournament_typeC(sport, tournament, country, SEASON, max_page=25):
    ...
    try:
        data_df.columns = ['TeamsRaw', 'Bookmaker', 'OddHome',
                           'OddDraw', 'OddAway', 'DateRaw', 'ScoreRaw']
    except:
        print('Function crashed, probable reason : no games scraped (empty season)')
        return (1)
    ##################### FINALLY WE CLEAN THE DATA AND SAVE IT ##########################
    ...
    data_df["Date"] = [re.split(', ', y)[1] for y in data_df["DateRaw"]]
    # (c) Split score
    data_df["Score_home"] = [re.split(':', y)[0][-2:]
                             for y in data_df["ScoreRaw"]]
    data_df["Score_away"] = [re.split(':', y)[1][:2]
                             for y in data_df["ScoreRaw"]]
    # (e) Set season column
    data_df["Season"] = SEASON
    # Finally we save results
...
def scrape_current_season_typeC(tournament, sport, country, SEASON, max_page=25):
    ...
    for page in range(1, max_page + 1):
        ...
        driver = webdriver.Chrome(executable_path=DRIVER_LOCATION) 
        data = scrape_page_current_season_typeC(
            page, sport, country, tournament)
        DATA_ALL = DATA_ALL + [y for y in data if y != None]
        driver.close()
    data_df = pd.DataFrame(DATA_ALL)
...
def scrape_league_typeC(Season, sport, country1, tournament1, nseason, current_season='yes', max_page=25):
    ...
    if current_season == 'yes':
        ...
        if long_season:
            SEASON1 = '{}-{}'.format(Season, Season+1)
        print('We start to collect current season')
        scrape_current_season_typeC(tournament=tournament1, sport=sport,
                                    country=country1, SEASON='CurrentSeason', max_page=max_page)
        print('We finished to collect current season !')
...
def scrape_next_games_typeC(tournament, sport, country, SEASON, nmax=30):
    ...
    data_df[['Home_id', 'Away_id', 'Bookmaker', 'OddHome', 'OddDraw', 'OddAway', 'Date', 'Score_home', 'Score_away', 'Season']].to_csv(
        './{}/NextGames_{}_{}.csv'.format(tournament, tournament, SEASON), sep=';', encoding='utf-8', index=False)
    ...
    if a == True:
        ...
        if a == True:
            ...
            for j in range(1, 15):  # only first 10 bookmakers displayed
                ...
                # first home odd
                Odd_1 = fffi(
                    '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[2]'.format(j))
                # first away odd
                Odd_2 = fffi(
                    '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[3]'.format(j))
                match = ffi('//*[@id="col-content"]/h1')  # match teams
                final_score = ffi('//*[@id="event-status"]')
                date = ffi('//*[@id="col-content"]/p[1]')  # Date and time
                print(match, Book, Odd_1, Odd_2,
...
def scrape_current_tournament_typeD(sport, tournament, country, SEASON, max_page=25):
    ...
    data_df["Date"] = [re.split(', ', y)[1] for y in data_df["DateRaw"]]
    # (c) Split score
    data_df["Score_home"] = [re.split(':', y)[0][-1:]
                             for y in data_df["ScoreRaw"]]
    data_df["Score_away"] = [re.split(':', y)[1][:1]
                             for y in data_df["ScoreRaw"]]
    # (e) Set season column
    data_df["Season"] = SEASON
    if not os.path.exists('./{}'.format(tournament)):
        os.makedirs('./{}'.format(tournament))
    data_df[['Home_id', 'Away_id', 'Bookmaker', 'OddHome', 'OddAway', 'Date', 'Score_home', 'Score_away', 'Season']].\
        to_csv('./{}/{}_{}_08042020.csv'.format(tournament, tournament,
               SEASON), sep=';', encoding='utf-8', index=False)
    return (data_df)
...
def scrape_current_season_typeD(tournament, sport, country, SEASON, max_page=25):
    ...
    for page in range(1, max_page + 1):
        ...
        driver = webdriver.Chrome(executable_path=DRIVER_LOCATION)
        data = scrape_page_current_season_typeD(
            page, sport, country, tournament)
        DATA_ALL = DATA_ALL + [y for y in data if y != None]
        driver.close()
    ...
    data_df[['Home_id', 'Away_id', 'Bookmaker', 'OddHome', 'OddAway', 'Date', 'Score_home', 'Score_away', 'Season']].\
        to_csv('./{}/{}_CurrentSeason_08042020.csv'.format(tournament,
               tournament), sep=';', encoding='utf-8', index=False)
    #
...
def scrape_league_typeD(Season, sport, country1, tournament1, nseason, current_season='yes', max_page=25):
    ...
    if current_season == 'yes':
        ...
        if long_season:
            SEASON1 = '{}-{}'.format(Season, Season+1)
        print('We start to collect current season')
        scrape_current_season_typeD(sport=sport, tournament=tournament1,
                                    country=country1, SEASON='CurrentSeason', max_page=max_page)
        print('We finished to collect current season !')
...
def scrape_oddsportal_current_season(sport='football', country='france', league='ligue-1', season='2019-2020', max_page=25):
    ...
    L = ['soccer', 'basketball', 'esports', 'darts', 'tennis', 'baseball', 'rugby-union',
         'rugby-league', 'american-football', 'hockey', 'volleyball', 'handball']
    ...
    if sport in ['baseball', 'esports', 'basketball', 'darts', 'american-football', 'volleyball']:
        df = scrape_current_season_typeA(
            tournament=league, sport=sport, country=country, SEASON=season, max_page=max_page)
    ...
    df.to_csv('./{}/CurrentSeason_{}_{}_{}.csv'.format(sport, country,
              league, season), sep=',', encoding='utf-8', index=False)

def scrape_oddsportal_historical(sport='football', country='france', league='ligue-1', start_season='2010-2011', nseasons=1, current_season='yes', max_page=25):
    ''' sport : sport as mentioned on the oddsportal website 
        country : country as mentioned on oddsportal website 
        league : league as mentioned on oddsportal website
    ...
    if sport in ['soccer', 'rugby-union', 'rugby-league', 'handball']:
        df = scrape_league_typeC(Season=start_season, sport=sport, country1=country,
                                 tournament1=league, nseason=nseasons, current_season=current_season, max_page=max_page)
        df = create_clean_table_three_ways(df)
    ...
    df.to_csv('./{}/Historical_{}_{}.csv'.format(sport, country,
              league), sep=',', encoding='utf-8', index=False)
...
def scrape_oddsportal_next_games(sport='football', country='france', league='ligue-1', season='2019-2020', nmax=30):
    ...
    if sport in ['baseball', 'esports', 'basketball', 'darts', 'american-football', 'volleyball']:
        ...
        df = scrape_next_games_typeB(Surface=surface, bestof=bestof, tournament=league,
                                     country=country, name_to_write=league, SEASON='2020')
        df = create_clean_table_two_ways(df)
    elif sport in ['soccer', 'rugby-union', 'rugby-league', 'handball']:
    ...
    df.to_csv('./{}/NextGames_{}_{}_{}.csv'.format(sport, country,
              league, season), sep=',', encoding='utf-8', index=False)
...
def scrape_oddsportal_specific_season(sport='football', country='france', league='ligue-1', season='2019-2020', max_page=25):
    ...
    if sport in ['baseball', 'esports', 'basketball', 'darts', 'american-football', 'volleyball']:
        df = scrape_current_tournament_typeA(
            sport=sport, tournament=league, country=country, SEASON=season, max_page=max_page)
        df = create_clean_table_two_ways(df)
    # elif sport in ['tennis']:
    ...
    df.to_csv('./{}/Season_{}_{}_{}.csv'.format(sport, country,
              league, season), sep=',', encoding='utf-8', index=False)
...
def reject_ads(switch_to_decimal=True):
    ...
    if switch_to_decimal and (driver.find_element("xpath", '//*[@id="app"]/div/header/div[2]/div[1]/div/button/p').text != "EU Odds" ):        
        # Change odds to decimal format
        driver.find_element("xpath", '//*[@id="app"]/div/header/div[2]/div[1]/div/button').click() 
        driver.find_element("xpath", '//*[@id="app"]/div/header/div[2]/div[1]/div/div/ul/li[1]').click()
